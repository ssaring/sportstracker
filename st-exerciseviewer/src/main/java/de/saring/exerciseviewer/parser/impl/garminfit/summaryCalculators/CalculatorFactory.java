    ...
    public Collection<ExerciseSummaryAggregator> createCalculators(EVExercise exercise) {
        ...
        if (exercise.getRecordingMode().isAltitude()) {
            if (exercise.getAltitude() == null
                || altitudeSummaryNotCalculated(exercise.getAltitude())) {
            result.add(new AltituteSummaryCalculator(exercise));
        }
        }
        return result;
    }

    private boolean altitudeSummaryNotCalculated(ExerciseAltitude altitude) {
        return altitude.getAltitudeMin() == 0 || altitude.getAltitudeAVG() == 0 || altitude.getAltitudeMax() == 0;
    }
}
